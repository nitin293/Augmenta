...
def scaleIMG(image):
    ...
    input_img = imaugs.scale(input_img_path, factor=0.3)    
...
def main(img, filt_name):
    ...
    if filt_name == "overlay":
        aug_img = overlay(image=img, emojipath=emojipath, fontfiles=fontfiles, use_ftr=2)    # out of 2 filters

    elif filt_name == "colortransform":
        aug_img = colorTransform(img, use_ftr=3)    # out of 3
    
    elif filt_name == "spatialtransform":
        aug_img = spatialTransform(img, imgpath, use_ftr=7)     # out of 7
        
    elif filt_name == "pixelleveltransform":
        aug_img = pixelLevel_transform(img, eff_num=3)      # out of 3
...
#     -------------- output environment setup --------------------
    ...
    for setdir in ["set" + str(i) for i in range(1, 6)]:
        if setdir not in os.listdir(outputpath):
            os.mkdir(outputpath + setdir)
...
#     ------------------------- automation -----------------------
    ...
    output_list = glob.glob(outputpath + '*')
    
        
    #for out in output_list:
    for i in range(len(effect_classes)-1):
        out = output_list[i]
        ...
        for img in img_list:
            ...
            aug_img = main(img, effect_classes[effect])
